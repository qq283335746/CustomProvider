...
namespace Yibi.LiteMembershipProvider
{
    public class AspnetSecurityService
    {
        ...
        public void RemoveUsersFromRoles(Guid applicationId, string[] usernames, string[] roleNames)
        {
            ...
            _db.UsersInRoles.Delete(m => roleids.Contains(m.RoleId) && userids.Contains(m.UserId));
        }

        public bool RoleExists(Guid applicationId, string roleName)
        {
            return _db.Roles.Exists(m => m.ApplicationId.Equals(applicationId) && m.Name.Equals(roleName, StringComparison.OrdinalIgnoreCase));
        }
